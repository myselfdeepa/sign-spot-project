...
def viz_similarities(
    ...
    labels = None
    codec = None
    # Support extra arguments via global or monkey patch
    if hasattr(viz_similarities, "extra_args"):
        labels = viz_similarities.extra_args.get("labels", None)
        codec = viz_similarities.extra_args.get("codec", None)
    """
    ...
    for t in tqdm(range(num_frames)):
        ...
        if labels is not None:
            predicted_class = int(np.argmax(sim[t_ix]))
            if 0 <= predicted_class < len(labels):
                label_text = labels[predicted_class]
            else:
                label_text = f"class {predicted_class}"
            rect = plt.Rectangle((offset, 200), 180, 40, linewidth=0, edgecolor='none', facecolor='black', alpha=0.6)
            ax1.add_patch(rect)
                
            ax1.text(
                offset,
                240,  # A little above the keyword line
